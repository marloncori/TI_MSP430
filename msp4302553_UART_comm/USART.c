...
{
    /* declaracion de registro, pensado para realizar las operaciones directamente en un registro,
    ...
    **/
     ...
     *el manejo de direccion e interrupciones ser�n en otro video
     ...
     * Suponiendo un reloj de 16MHz
     */
    fBaudRateDecimal= __CLK_FREQ/(float)s32BaudRate;
    //se obtiene la divison necesaria para obtener el baud rate deseado

    /*
     * NOTA: esta aproximacion no es la optima , para obtener el menor error
    ...
    /*Confguracion de las interrupciones del UARt, no se vera hasta en proximos videos
     * UCA0RXIE =0 (deshabilitadas interrupcion de recepcion)
     * UCA0TXIE=0  (deshabilitadas interrupcion de trnasmision)
     */
    /* Configuracion final para Uart0 Control (UC0CTL0)
    ...
    /* Configuracion final para Uart0 Control (UC0CTL1)
