...
USART_Init: bis.b	#UCSWRST, &UCA0CTL1		; enable SW reset
			bis.b	#UCCKPL+UCMSB+UCMST+UCSYNC, &UCA0CTL0
...
MainLoop:	mov.w	#2, R7					; load R7 with two bytes
			bis.w	#LPM0, SR
			jmp		MainLoop
...
;-------------------------------------------
			mov.b	UCA0RXBUF, R6			; move received data from slave to R6
			tst.w	R7						; check if R7 is zero
			jne		EndISR2					; if not jmp to EndISR2
...
P1_ISR		;
;-------------------------------------------
			bic.b	#UCA0TXIE, &IE2			; enable tx interrupt
			clr.b	P1IFG
			reti
;-------------------------------------------------------------------------------
